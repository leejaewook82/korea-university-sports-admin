...
import { getDatabase, saveDatabase } from './src/server/db';
import { Athlete, ExpensePlan, TripRequest, TripReport, ApprovalLine, Receipt, User } from './src/types';
...
function getGeminiClient(): GoogleGenAI {
  ...
  if (!apiKey) {
    throw new Error('GEMINI_API_KEY가 환경 변수에 설정되어 있지 않습니다. AI Studio UI에서 Secrets를 확인해 주세요.');
  }
  if (!aiClient) {
...
async function startServer() {
  ...
  // Increase payload limit to handle photo uploads via base64 JSON
  ...
  // Expense Management (훈련 및 대회 경비 지출)
  ...
  // Business Trips: Requests (지도자 출장 계획서)
  ...
  // Business Trips: Reports (지도자 출장 보고서)
  ...
  // Base64 File Upload endpoint (zero multi-part dependency approach)
...
영수증의 글씨가 뭉개져서 읽지 못한 필드가 있다면 빈 문자열("") 또는 0(금액의 경우)으로 전달해 주세요.`,
  ...
  // Serve static UI assets in production or fallback to Vite in dev
